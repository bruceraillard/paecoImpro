...
function persistSettings() {
    Browser.writeStorageJson(localStorage, Game.STORAGE_KEY, gameState.settings);
}
...
function bindKeyboardShortcuts() {
    document.addEventListener('keydown', event => {
        if (Browser.isTypingTarget(event.target) || event.metaKey || event.ctrlKey || event.altKey) return;
